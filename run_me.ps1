...
param ([string]$SourcePath, [String] $vmName="New-VM", [String] $server="localhost", [string]$vhdpath, [string]$mem = 2GB, [int]$gen=1, [string]$locpath="C:\Hyper-V")
...
function SendToVM([string]$SourcePath, [String] $vmName, [String] $server, [String] $IP){
    $ipAux = "$IP" + ":"
    ./pscp.exe -pw Passw0rd $SourcePath"\AIO.sh" root@$ipAux
    ./pscp.exe -pw Passw0rd $SourcePath"\rhel5_id_rsa.pub" root@$ipAux
    ./pscp.exe -pw Passw0rd $SourcePath"\rhel5_id_rsa" root@$ipAux
}
...
Function CreateVM([string]$vhdpath, [string]$vmName, [string]$mem, [int]$gen, [string]$locpath){
    if(($vhdpath.length -eq 0)){
    ...
    New-VM -Name $vmName -MemoryStartupBytes $mem -Generation $gen -VHDPath $vhdpath -Path $locpath
...
CreateVM $vhdpath $vmName $mem $gen $locpath
...
SendToVM $SourcePath $vmName $server $ip
